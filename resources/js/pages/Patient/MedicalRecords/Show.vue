...
<template>
    <div>
        <div class="mb-6">
            <a href="/patient/records" class="text-sm text-gray-400 hover:text-gray-600">&larr; All Records</a>
            <h1 class="text-2xl font-bold text-gray-900 mt-1">Medical Record</h1>
            <p class="text-sm text-gray-500">{{ formatDate(record.visit_date) }} &middot; Dr. {{ record.doctor?.user?.name }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <div>
                    <p class="text-xs font-medium text-gray-400 uppercase">Doctor</p>
                    <p class="text-sm text-gray-900">Dr. {{ record.doctor?.user?.name }}</p>
                </div>
